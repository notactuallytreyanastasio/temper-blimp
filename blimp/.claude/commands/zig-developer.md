...
Follow the same red-green-refactor loop as the Elixir side:

1. **Red** - Write a failing `test` block that describes the behavior you want
2. **Green** - Write the minimum code to make `zig build test` pass
3. **Refactor** - Clean up, extract, simplify. Tests still pass.
4. Repeat.

```bash
zig build test    # Run all tests
zig build         # Clean compile check
```
